...
if [ -n "$SIG_AUTH" ]; then
  echo "  подпись: $SIG_AUTH (оставлена как есть)"
  codesign --verify --strict "$INSTALLED" 2>&1 | head -1
else
  # Запасной путь, пока сертификата нет: ad-hoc с УСТОЙЧИВЫМ идентификатором. Линкер выводит
  # идентификатор из хеша бинарника, и после каждой пересборки macOS видит новую программу —
  # отсюда окно с паролем от связки. Закрепление идентификатора это смягчает, но не лечит:
  # у ad-hoc требование строится на `cdhash`, а он меняется всё равно (см. Р-8 в журнале).
  codesign -f -s - -i plus.flamingo.desktop --deep "$INSTALLED" 2>/dev/null \
    && echo "  подпись: ad-hoc, идентификатор plus.flamingo.desktop" \
    || echo "  ⚠️ подписать не удалось"
fi
