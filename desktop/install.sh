...
# Настоящая подпись, если она уже есть, — НЕ ТРОГАТЬ. `codesign -f -s -` поверх Developer ID
# затёр бы её ad-hoc-подписью, и вся работа по нотаризации ушла бы в песок незаметно.
# 🔴 Подпись обязана быть настоящей (промпт 23 §2.3). Ad-hoc ветка убрана: сертификат
# выпущен, и «поставим пока без подписи» теперь означает молча отдать владельцу сборку,
# которая при каждом обновлении снова начнёт спрашивать пароль от связки (Р-8).
SIG_AUTH="$(codesign -dv --verbose=2 "$INSTALLED" 2>&1 | grep -m1 '^Authority=' | cut -d= -f2-)"
case "$SIG_AUTH" in
  "Developer ID Application"*)
    echo "  подпись: $SIG_AUTH"
    codesign --verify --strict "$INSTALLED" 2>&1 | head -1
    xcrun stapler validate "$INSTALLED" >/dev/null 2>&1 \
      && echo "  нотаризация: талон на месте" \
      || echo "  ⚠️ талона нет — на чужой машине будет «неизвестный разработчик»"
    ;;
  *)
    echo "  ⚠️ ПОДПИСЬ НЕ DEVELOPER ID: ${SIG_AUTH:-нет подписи}"
    echo "     Соберите и подпишите: bash desktop/sign-and-notarize.sh"
    echo "     Ставить так можно только для себя и только зная, зачем."
    ;;
esac
