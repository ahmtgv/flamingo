...
# 🔴 ОДНА ВЕРСИЯ — ЖЁСТКО (§39.1, наряд 37 §2). Копию из каталога сборки убираем СРАЗУ:
# пока она там лежит, Launchpad показывает два одинаковых значка и система выбирает наугад.
# Артефакт для раздачи — образ `.dmg`, он остаётся; запускаемой копии в репозитории нет.
