...
# 🔴 §54.1.3: КРАСНЫЙ СЦЕНАРИЙ БОЛЬШЕ НЕ МОЛЧИТ.
#
# За неделю трижды находили караул, простоявший в красном неизвестно сколько: `live`,
# `site`, `wizard`. «Тесты зелёные» значило только `vitest`; `e2e` не запускал никто.
# Теперь проверки идут ПЕРЕД сборкой, и красное останавливает выпуск — раньше, чем образ
# уедет владельцу.
#
# Пропустить можно осознанно: `SKIP_CHECKS=1 bash desktop/release.sh` — но это видно в
# команде, а не молчаливо.
if [ "${SKIP_CHECKS:-}" = "1" ]; then
  echo "── проверки пропущены по требованию (SKIP_CHECKS=1) ──"
else
  step "Проверки" bash -c "cd '$REPO/frontend' && npm run check"
fi
