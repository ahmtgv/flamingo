...
#[tauri::command]
fn read_machine_key(app: tauri::AppHandle) -> Option<String> {
    // 🔴 Исход чтения — в лог, дословно и без самого ключа (Р-7/Р-8). Именно этот вызов решает,
    // считает ли приложение себя связанной машиной; когда он молча отдавал `None`, отличить
    // «ключа нет» от «ключ есть, но шаг мастера первый» на экране было невозможно — оба
    // выглядели как шаг 1 со свежим кодом.
    match keyring().and_then(|e| e.get_password().map_err(|x| x.to_string())) {
        Ok(key) => {
            note(&app, &format!("ключ машины: прочитан из связки, длина {}", key.len()));
            Some(key)
        }
        Err(e) => {
            note(&app, &format!("ключ машины: не прочитан — {e}"));
            None
        }
    }
}
