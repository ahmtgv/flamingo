...
async function mint({ request, env }) {
  ...
  // 🔴 КЛЮЧ ПРОВЕРЯЕТСЯ НАРАВНЕ С ДВУМЯ ДРУГИМИ. Стояло два из трёх, и при заданных
  // адресе и секрете, но пустом ключе ниже подставлялся издатель 'devkey' — токен
  // выписывался НАСТОЯЩИМ секретом от несуществующего издателя, облако его отклоняло,
  // и человек видел «подключаемся» без единого слова о причине. Ровно то, чего
  // соседний комментарий обещал не допустить. Найдено 08.09, та же мина на сервере.
