...
const МЕТОДИЧКИ: { имя: string; ждёт?: string; экран?: true; пособия?: true }[] = [
  ...
  /* 🔴 «Материалы для урока» БОЛЬШЕ НЕ ПРИГЛУШЕНЫ, когда они есть. Дверь
     стояла с подписью «появятся вместе с курсом» — а они уже пришли: их
     кладут на экране «Создать урок». Оставить надпись значило бы врать
     собственному преподавателю про его же файлы. Ведёт в тот же список, что
     и «Учебные Документы»: два имени одной полки — не беда, а привычка;
     беда — дверь, которая молчит о том, что открылась. */
  { имя: 'Материалы для урока', пособия: true,
