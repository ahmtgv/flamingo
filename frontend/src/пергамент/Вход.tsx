/** ВНЕ ПРОДУКТА: решение владельца, а не забытый файл. 04.09: «не уверен,
 *  что вывезешь без потерь — буду думать». Пергаментный вход цел и ждёт;
 *  чтобы вернуть его, в `App.tsx` меняется одна строка ввоза и одно имя.
 *  Набор `пергамент.css` и его караулы остаются в проекте: пока ни один экран
 *  не помечен `data-язык="пергамент"`, они ни на что не действуют.
 */
        ...
        <section className={s.карточка}>
          ...
          <form className={s.форма} onSubmit={(e) => { e.preventDefault(); войти() }}>
            <div className={s.поле}>
              ...
              />
              {/* 🔴 ОДНО МЕСТО НА ДВОИХ. Пока всё хорошо — здесь подсказка; случился
                  отказ — на её месте отказ. Высота держится по самому длинному из
                  двух, поэтому кнопка не сдвигается (ПРАВИЛА 6.6), а пустоты под
                  полем нет: место всегда чем-то занято. */}
              <div className={s.подПолем} data-geo="строка-сообщения" role="status">
                {сказано ? (
                  <span className={s.речь}>
