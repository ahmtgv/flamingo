...
export type Урок = {
  ...
  название: string
  дата: string
  /** Настенные часы. Чьи именно — говорит `пояс`. */
  время: string
  /** Пояс, в котором заданы `дата` и `время` (имя IANA). Пусто — не знаем,
   *  и тогда экран не имеет права утверждать, чьё это время. */
  пояс?: string
  минут: number
  код: string
  материалы: Пособие[]
...
export type Журнал = {
  уроки: { id: string; дата: string; время: string; пояс?: string
           название: string; код: string; прошёл: boolean }[]
 ...
 *
 *  Пояс у местного урока — пояс этого браузера: занятие завёл человек, сидящий
 *  здесь, других сведений нет и быть не может. */
function изМестного(у: местные.Урок): Урок {
  return { ...у, пояс: у.пояс ?? мойПояс(), материалы: [] }
}
...
export async function завестиУрок(что: {
  ...
  /** Только для стенда: подложить чужой пояс, чтобы увидеть пересчёт. */
  пояс?: string
}): Promise<Урок> {
  try {
    /* Пояс уходит вместе с уроком: сервер возьмёт пояс преподавателя, а если
       он там ещё не известен — этот. Другого случая узнать может не быть. */
    const r = await json<{ урок: Урок }>('lessons', 'POST', { ...что, tz: мойПояс() })
    дом = 'сервер'
    return r.урок
  } catch (e) {
    if (e instanceof Беда) throw e
...
//
// Здесь нет отхода в браузер, в отличие от расписания. Переписка — разговор
// двух людей: сохранить её «пока только у меня» значит показать человеку
// отправленное сообщение, которого второй не получит. Молчит сервер —
// говорим словами.

export type Реплика = {
  id: string
  /** Моё ли это сообщение. Считает сервер: он знает, кто прислал куку. */
  мой: boolean
  вид: 'text' | 'invite'
  текст: string
  когда: string
  прочитано: boolean
  /** Только у зова. `null` — занятие сняли, а сказанное осталось. */
  урок?: { id: string; название: string; дата: string; время: string; пояс?: string
           код: string } | null
}

export type Разговор = {
  кто: string
  имя: string
  роль: string
  с: string
  непрочитано: number
  последнее: Реплика | null
}

/** С кем я в переписке.
 *
 *  `null` — сервер МОЛЧИТ (`НетСервера`): сказать нечего, потому что слов не
 *  было. Отказ со словами (`Беда`) летит наружу и показывается словами.
 *
 *  🔴 РАНЬШЕ ЗДЕСЬ БЫЛО `catch { return null }` НА ОБА СЛУЧАЯ, И ЭТО СТОИЛО
 *  ЭКРАНА. Кабинет ученика рисует `null` как «Смотрим, с кем вы в переписке…» —
 *  то есть как ЗАГРУЗКУ. Сервер отвечает 403 со словами «сессия истекла» —
 *  ученик видит вечное «смотрим» и ждёт. Отказ, молчание и загрузка — три
 *  разных состояния (ПРАВИЛА 6.1), и сваливать их в одно значит соврать
 *  человеку о том, что происходит. */
export async function разговоры(): Promise<Разговор[] | null> {
  try {
    const r = await разговор<{ разговоры: Разговор[] }>('talks')
    дом = 'сервер'
    return r.разговоры
  } catch (e) {
    if (e instanceof Беда) throw e
...
/** Лента разговора. `null` — сервер молчит. Отказ со словами летит наружу:
 *  панель показывала «Сервер не отвечает» даже когда сервер ОТВЕТИЛ и объяснил,
 *  почему нельзя. */
export async function лента(кто: string): Promise<{
  имя: string; роль: string; письма: Реплика[]
} | null> {
  try {
    return await разговор<{ имя: string; роль: string; письма: Реплика[] }>(
      `talks/${encodeURIComponent(кто)}`)
  } catch (e) {
    if (e instanceof Беда) throw e
    return null
  }
...
/** Написать словами. Отказ сервера долетает словами же (`Беда`). */
export async function написать(кто: string, текст: string): Promise<Реплика> {
  const r = await json<{ письмо: Реплика }>(`talks/${encodeURIComponent(кто)}`, 'POST', { текст })
  return r.письмо
}

/** Позвать на занятие. Сервер сам проверит, что занятие моё. */
export async function позвать(кто: string, урок: string): Promise<Реплика> {
  const r = await json<{ письмо: Реплика }>(`talks/${encodeURIComponent(кто)}`, 'POST', { урок })
  return r.письмо
}

/** Отметить прочитанным всё, что пришло от этого человека.
 *  🔴 Отдельным вызовом, а не открытием ленты: чтение не должно менять
 *  состояние, иначе предзагрузка страницы гасит непрочитанное. */
export async function прочитано(кто: string): Promise<void> {
  try {
    await json<{ отмечено: number }>(`talks/${encodeURIComponent(кто)}/read`, 'POST', {})
  } catch {
    /* Не отметилось — не беда урока: в следующий раз отметится. */
  }
}
