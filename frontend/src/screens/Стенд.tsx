...
export const ЭКРАНЫ: { имя: string; путь: string; рисуй: () => React.ReactElement }[] = [
  ...
  /* 🔴 РАМА — ЭТО ТОЖЕ ЭКРАН. Постоянная навигация (решение владельца 09–10.09)
     живёт вокруг кабинета, журнала и источников; посмотреть на неё без сервера
     было негде, а экран, которого нет на стенде, невидим для проверки. */
  { имя: 'рама-кабинет', путь: '/кабинет', рисуй: () => (
    <Рама person={УЧИТЕЛЬ} где="кабинет" onGo={ни} onOut={ни}>
      <СЖурналом дети={<Cabinet person={УЧИТЕЛЬ} onLesson={ни} onNew={ни} onEdit={ни} onJournal={ни} onOut={ни} onHome={ни} вРаме />} />
    </Рама>
  ) },
  { имя: 'рама-кабинет-ученик', путь: '/кабинет', рисуй: () => (
    <Рама person={УЧЕНИК} где="кабинет" onGo={ни} onOut={ни}>
      <СЖурналом дети={<Cabinet person={УЧЕНИК} onLesson={ни} onNew={ни} onEdit={ни} onJournal={ни} onOut={ни} onHome={ни} вРаме />} />
    </Рама>
  ) },
  { имя: 'рама-журнал', путь: '/журнал', рисуй: () => (
    <Рама person={УЧИТЕЛЬ} где="журнал" onGo={ни} onOut={ни}>
      <Journal person={УЧИТЕЛЬ} onBack={ни} onHome={ни} onOut={ни} onNew={ни} onLesson={ни}
               подложка={ЖУРНАЛ_БОЛЬШОЙ} вРаме />
    </Рама>
  ) },
  { имя: 'рама-источники', путь: '/hub', рисуй: () => (
    <Рама person={УЧИТЕЛЬ} где="избранное" onGo={ни} onOut={ни}>
      <Hub onBack={ни} onHome={ни} вРаме />
    </Рама>
  ) },
