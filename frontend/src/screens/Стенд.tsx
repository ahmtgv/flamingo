...
const РАЗГОВОР = (яУчитель: boolean): Реплика[] => {
  ...
  return [
    ...
    { id: 'e', вид: 'invite', текст: '', когда: час(3), мой: от(true), прочитано: false,
      урок: { id: 'у9', название: 'Английский · разговор', дата: '2026-09-09', время: '10:00', код: 'проба' } },
  ] as Реплика[]
}
...
async function положитьУроки(сколько: number, поМесяцу: boolean) {
...
function useПодложенныеУроки(нужны: boolean, сколько = 3, поМесяцу = false) {
  const [готово, setГотово] = useState(false)
  useEffect(() => {
    if (!нужны || готово) return
    let живо = true
    const метка = `${сколько}/${поМесяцу}`
    if (!раскладка || раскладка.метка !== метка) {
      раскладка = { метка, ждём: положитьУроки(сколько, поМесяцу) }
    }
    раскладка.ждём.then(() => { if (живо) setГотово(true) })
    return () => { живо = false }
  }, [нужны, готово, сколько, поМесяцу])
  return готово
}

function СЖурналом({ дети, сколько, поМесяцу }: {
  дети: React.ReactNode; сколько?: number; поМесяцу?: boolean
}) {
  const готово = useПодложенныеУроки(true, сколько, поМесяцу)
