...
export function доска(c: HTMLCanvasElement, П: Палитра) {
  ...
  if (!g) return {
    ...
    имена: [] as string[],
    взятьПеро: () => undefined,
    вести: (_x: number, _y: number, _ц: string) => undefined,
    отпустить: () => undefined,
    сторож: (_н?: (n: number) => void) => undefined,
    вРуке: () => false,
  ...
  let поколение = 0
  let текущая = 0
  /* 🔴 ВЗЯЛИ ПЕРО — СЦЕНЫ МОЛЧАТ. Иначе следующий кадр сцены стирает холст
     вместе с тем, что человек только что нарисовал. */
  let режим: 'сцены' | 'рука' = 'сцены'
  let последний = 0
  /* Точки текущего штриха человека. Нужны ТРИ подряд: середины соседних
     отрезков дают гладкую кривую без изломов. */
  let штрих: Точка[] = []
  let таетЗапущен = false
  let жив = true

  /* Чернила руки медленно тают: доска не превращается в свалку и всегда
     возвращается к показу сама. */
  function таять() {
    g!.globalCompositeOperation = 'destination-out'
    g!.fillStyle = 'rgba(0,0,0,.0016)'
    g!.fillRect(0, 0, W(), H())
    g!.globalCompositeOperation = 'source-over'
  }
  function кадрРуки() {
    if (!таетЗапущен || !жив) return
    if (режим === 'рука') таять()
    requestAnimationFrame(кадрРуки)
  }
  ...
  /** 🔴 РУКА ЧЕЛОВЕКА РИСУЕТСЯ КАК НА НАСТОЯЩЕЙ ДОСКЕ, А НЕ ПОЧЕРКОМ СЦЕН.
   *
   *  Дрожь, гуляющий нажим и перелёт — это то, чем МАШИНА притворяется рукой.
   *  Живой руке они не нужны: она уже рука, и от них штрих становился ломаным
   *  и дёрганым. Здесь, как в `board/Board.tsx`: постоянная толщина, круглые
   *  концы и стыки.
   *
   *  Гладкость даёт квадратичная кривая по СЕРЕДИНАМ соседних отрезков: точки
   *  от указателя приходят редко, и `lineTo` между ними виден углами — именно
   *  это и было на снимке. Середины прячут узлы внутрь кривой. */
  const сер = (a: Точка, b: Точка): Точка => [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2]
  function вести(x: number, y: number, цвет: string) {
    последний = Date.now()
    const p: Точка = [x, y]
    const пред = штрих[штрих.length - 1]
    if (пред) {
      /* Длинный прыжок линией не соединяем: иначе через всю доску тянется
         прочерк оттуда, где указатель был до того, как вернулся. */
      const d = Math.hypot(p[0] - пред[0], p[1] - пред[1])
      if (d > 240) { штрих = [p]; return }
      if (d < 0.01) return
    }
    штрих.push(p)
    const n = штрих.length
    if (n < 2) return
    g!.globalCompositeOperation = 'source-over'
    g!.strokeStyle = цвет
    g!.lineWidth = 3.4
    ...
    g!.globalAlpha = 1
    g!.beginPath()
    if (n === 2) { g!.moveTo(штрих[0][0], штрих[0][1]); g!.lineTo(штрих[1][0], штрих[1][1]) }
    else {
      const a = штрих[n - 2], b = штрих[n - 1], c = штрих[n - 3]
      const м1 = сер(c, a), м2 = сер(a, b)
      g!.moveTo(м1[0], м1[1])
      g!.quadraticCurveTo(a[0], a[1], м2[0], м2[1])
    }
    g!.stroke()
  }
  function взятьПеро() {
    if (режим === 'рука') return
    режим = 'рука'
    поколение += 1
    чисто()
    штрих = []
    if (!таетЗапущен) { таетЗапущен = true; кадрРуки() }
    последний = Date.now()
  }
  function отпустить() {
    /* Хвост: последняя середина не дотягивается до самой точки отпускания. */
    const n = штрих.length
    if (n >= 2) {
      const a = штрих[n - 2], b = штрих[n - 1]
      g!.beginPath(); g!.moveTo(...сер(a, b)); g!.lineTo(b[0], b[1]); g!.stroke()
    }
    штрих = []
  }
  /** Восемь секунд тишины — и доска снова показывает сама. */
  function сторож(наСмену?: (n: number) => void) {
    if (!жив) return
    if (режим === 'рука' && Date.now() - последний > 8000) {
      режим = 'сцены'
      чисто()
      играть(текущая + 1, наСмену)
    }
    window.setTimeout(() => сторож(наСмену), 700)
  }
  ...
  return {
    ...
    играть,
    /* Экран ушёл — умолкают все три петли: сцена, таяние и сторож. Без этого
       они продолжали рисовать в оторванный от страницы холст. */
    стоп: () => { жив = false; поколение += 1; чисто() },
    текущая: () => текущая,
  }
}
