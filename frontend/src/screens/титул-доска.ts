...
export function доска(c: HTMLCanvasElement, П: Палитра) {
  ...
  /** Человек ведёт пером. Отрезок рисуется тем же почерком, что и сцены. */
  function вести(x: number, y: number, цвет: string) {
    последний = Date.now()
    const p: Точка = [x, y]
    if (рука) {
      const d = Math.hypot(p[0] - рука[0], p[1] - рука[1])
      /* Длинный прыжок мыши линией не соединяем: иначе через всю доску
         тянется прочерк оттуда, где указатель был до того, как вернулся. */
      if (d <= 90 && d > 0.01) линия([рука, p], 1, цвет, 3.4, Math.round(x + y))
    }
    рука = p
  }
  function взятьПеро() {
