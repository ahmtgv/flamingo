...
/* 🔴 ИМЯ ЖИВЁТ В ОДНОЙ СТРОКЕ КОЛОНКИ 96 px — это 14 знаков кеглем 13, и
   мерка проверена на 1280, 1440 и 1728: обрезанных нет ни на одном. Длиннее
   писать нельзя не из вкуса, а из арифметики: вторая строка делает карточку
   выше, а её превью — мельче соседних. */
const РОЛИКИ = [
  { файл: 'short-1', имя: 'Песок и звук' },
  { файл: 'short-2', имя: 'Тело на склоне' },
  { файл: 'short-3', имя: 'Синус на круге' },
  { файл: 'short-4', имя: 'Медный мотор' },
  { файл: 'short-5', имя: 'Волна из шаров' },
  { файл: 'short-6', имя: 'Путь планет' },
] as const
...
export function Титул({ onSign, onNew, onHub, молчит = false, onAgain }: {
  ...
  const подогнать = useCallback(() => {
    ...
    if (window.matchMedia('(max-width: 720px)').matches) {
      б.style.display = ''
      пр.style.width = ''
      const т = п.parentElement as HTMLElement | null
      if (т) т.style.width = ''
      return
    }
    пр.style.width = ''
    ...
    /* Ролику достаётся вся высота, что осталась под подписью, а ширина идёт
       за ней по 16:9 — пока не упрётся в ширину колонки. Больше уже некуда.
       🔴 ДВА ПРОХОДА, А НЕ ОДИН. Подпись стоит НАД кадром и по ширине равна
       ему: её высота зависит от ширины кадра, а ширина кадра — от её высоты.
       Первый проход меряет подпись во всю колонку (это верхняя оценка её
       высоты), второй — уже по найденной ширине. Иначе «2 минуты» уезжает
       к дальнему краю колонки, за 130 px от кадра. */
    const тек = п.parentElement as HTMLElement | null
    let ширина = колW
    for (let проход = 0; проход < 2; проход += 1) {
      if (тек) тек.style.width = `${ширина}px`
      const высота = остаток - п.getBoundingClientRect().height - 8
      ширина = Math.floor(Math.max(150, Math.min(колW, высота * 16 / 9)))
    }
    if (тек) тек.style.width = `${ширина}px`
    пр.style.width = `${ширина}px`
  }, [])
    ...
    <main className={s.экран}>
      ...
      <div className={s.разворот}>
        <div className={s.левая} ref={левая}>
          {/* 🔴 ОДНА МЫСЛЬ ВМЕСТО ТРЁХ АБЗАЦЕВ (решение владельца 10.09). Было:
              заголовок, лид в три строки и служебная строка «кому» — вместе
              около 150 px высоты левой колонки. Владелец снял заголовок и свёл
              остальное в две фразы; освободившаяся высота ушла в проморолик,
              ради которого колонка и существует. Строка осталась одна, поэтому
              она же и заголовок страницы: `h1` на экране должен быть. */}
          <div className={s.слова} ref={слова}>
            <span className={s.бровь}>живой урок · образование без скуки</span>
            <h1 className={s.мысль}>
              Одна ссылка — и урок уже начался. Один кабинет — от первого урока
              до научной степени.
            </h1>
          </div>

          {/* 🔴 ПОДПИСЬ ПЕРЕЕХАЛА НАД РОЛИКОМ (решение владельца 10.09, показано
              стрелкой на снимке). Сбоку она отнимала у кадра треть ширины
              колонки, а раскладка «сбоку/стопкой» выбиралась замером — двух
              раскладок больше нет, есть одна: строка, под ней кадр во всю
              оставшуюся высоту. */}
          <div className={s.проБлок} ref={блок}>
            <span className={s.проТекст}>
              <span className={s.проПодпись} ref={подпись}>
                <span className={s.проСтрока}>
                  <span>Посмотрите, каким мы задумали Flamingo в далёком 2020-м</span>
                  <s>2 минуты</s>
                </span>
                <span className={s.проКринж}>Кринж присутствует</span>
              </span>
            </span>
      ...
      <div className={s.низ}>
        <div className={s.часть}>
          {/* 🔴 ПОДЗАГОЛОВОК СНЯТ (решение владельца 10.09) — и вместе с ним
              вернулась одна строка шапки у обеих половин низа: ступенька между
              ними снова 0, а высвободившиеся 35 px ушли обложкам. */}
          ...
          <div className={s.лента}>
            {РОЛИКИ.map((р) => (
              <div className={s.ролик} key={р.файл}>
                ...
                {/* 🔴 ПОДПИСЬ — РОВНО ОДНА СТРОКА (решение владельца 10.09), и это
                    же держит обложки одного размера. Пока имя могло встать в две
                    строки, у карточки с длинным именем нижний ряд был выше, а
                    обложке (9:16 от оставшейся высоты) доставалось меньше — на
                    1728 превью расходились по размеру между собой. Одна строка —
                    один размер у всех шести. Целиком имя называет `title`. */}
                <span className={s.имя} title={р.имя}>{р.имя}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={s.часть}>
          <div className={s.нШапка}>
            ...
            <span className={s.нПодпись} title="источники мира · открыты для урока">источники мира · открыты для урока</span>
