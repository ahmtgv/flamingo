...
test.describe('мастер первого запуска', () => {
  ...
  test('от чистой установки до кабинета', async ({ page }) => {
    ...
    // ⚠️ Приветствий два: преподавателю «Здравствуйте», ученику «Привет», и ещё два
    // варианта без имени. Ждём любой из них — проверяем, что открылся КАБИНЕТ, а не
    // конкретную формулировку: иначе тест ломается от правки текста, к делу не относящейся.
