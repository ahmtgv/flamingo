...
/**
 * 🔴 `/verify` УБРАН ИЗ СПИСКА, ПОТОМУ ЧТО ЭТОГО ЭКРАНА НЕТ (наряд 35, найдено 18.08).
 *
 * Проверка сертификата — обещание продукта: MVP заменяет NFT-сертификаты на PDF + QR, и QR
 * ведёт как раз сюда. Маршрута `/verify` в роутере нет ни одного; всё, что туда приходит,
 * ловит `path="*"` и уводит на корень.
 *
 * Два теста на этот адрес при этом ГОДАМИ ПРОХОДИЛИ: «открывается и не пуст» — потому что
 * непустой была страница, на которую переадресовали (раньше вход, теперь афиша); axe —
 * потому что проверял её же. Ровно тот случай, ради которого заведено правило «не оставлять
 * теста, который выглядит проверкой и ею не является»: тест не врал про экран, он смотрел
 * не на тот экран.
 *
 * Строку вернуть, когда экран появится. Пока он не построен, честнее пустое место, чем
 * зелёная галочка о нём.
 */
