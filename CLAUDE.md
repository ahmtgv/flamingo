...
- **Age adaptation**: set `data-mode="kids"` on the root for grades 1–4; size/leading/tap tokens cascade automatically.
- 🔴 **A repeated case is a list of entities with `id`s, never parallel arrays of labels** (designer's rule, handover 2026-08-21). When the same case repeats — a state, a branch, a row — model it as `[{ id, title, body, … }]` and key by `id`. Parallel arrays taken by index look identical on screen and drift silently: the designer's own sheet **showed the first case under the third case's name** — contrast fine, markup intact, no scrolling, and every word of it false. Nothing measures this; only the shape prevents it.
- 🔴 **Two muted inks, not one** (designer's rule 4.11). `--color-text-hint` = the reader loses nothing by skipping it; `--color-text-detail` = they would not learn something (a deadline, a count, a state, a name, a right). Choose by asking *what happens if this goes unread*. `--color-text-tertiary` is a dying alias for `hint` — it named a place in the hierarchy while the contrast floor stands on obligation to read, and it swallowed us three times running. New code never takes it; `staleTokens.test.ts` enforces that.
- 🔴 **`minmax(0, 1fr)`, not `1fr`** (designer's rule 9.8, handover 2026-08-21). A bare `fr` track has an `auto` minimum: it cannot become narrower than its longest unbreakable word. Every single-column grid declares `grid-template-columns: minmax(0, 1fr)`, and every card-button inside one declares `min-width: 0`. Without it the longest word stretches the column by min-content and the screen gets a horizontal scrollbar — caught by the `mobileScroll` instrument. This has already bitten twice by hand: the cabinet's week strip collapsed to 89 px for seven days, and the sign-in frame refused to fit 100dvh.
- **Transparency is not hierarchy** (rule 4.7). Never dim a control or its text with `opacity` — it eats contrast invisibly to the tokens and visibly to the eye (the board pilot at `opacity: .3` measured 1.53 against a 4.5 floor). Quieter is done with colour and weight; hover changes border colour, not alpha. Content-level adaptation (bigger icons, fewer words, subject colors) is per-component (see design system §6.1).
