...
if [ "$WHAT" = "all" ] || [ "$WHAT" = "site" ]; then
  ...
  if [ "$AHEAD" = "0" ]; then
    ...
    # 🔴 ВЫКАТ СОЛГАЛ ПРО САЙТ (наряд 54 §2, случай 24.08). `git push` провалился
    # («Password authentication is not supported»), а скрипт напечатал «✓ сайт отвечает»
    # и «Готово»: проверка ниже смотрит, жив ли домен, а не наш ли на нём коммит.
    #
    # Та же болезнь, ради которой в августе строили ворота свежести на сервере: «выкачено»
    # снова значило «команда не упала». Теперь провал отправки останавливает выкат — дальше
    # идти незачем, сайт всё равно останется прежним.
    if git push; then
      ok "отправлено"
    else
      bad "push не прошёл — сайт останется прежним, дальше идти незачем"
      echo
      echo "  Пароли GitHub больше не принимает. Два пути, оба разовые:"
      echo "    • GitHub Desktop: открыть репозиторий и нажать Push origin;"
      echo "    • либо SSH:  git remote set-url origin git@github.com:<владелец>/<репозиторий>.git"
      echo "      (ключ заводится один раз: ssh-keygen -t ed25519, затем ключ в GitHub → Settings → SSH keys)"
      exit 1
    fi
  fi
