...
class Lesson(models.Model):
    ...
    и рядом с ними — ПОЯС (`tz`), в котором занятие поставлено.

    Так и было предсказано в этом же комментарии до решения владельца от 08.09:
    «часовой пояс придёт тогда, когда придут ученики из другого пояса, — и тогда
    он станет полем занятия, а не свойством хранения». Пришёл; стал.

    Почему не один момент в UTC — три причины, и ни одна не про удобство:

    · перевод часов. «Каждый вторник 14:00 по Берлину» переживает переход на
      летнее время; тот же момент в UTC уезжает на час. В России перевода нет
      с 2014-го, у ученика за границей — есть;
    · намерение автора. Преподаватель поставил 14:00 по СВОИМ часам. Это факт
      о занятии, и хранить надо его, а не вычислять обратно;
    · из момента настенные часы автора всё равно не восстановить, если пояс не
      сохранён отдельно, — то есть поле нужно в любом случае.

    Пусто в `tz` — «не знаем», и экран тогда не имеет права утверждать, чьё это
    время (ветка «пояса учеников не пришли» на листах дизайна).
    """
    ...
    on = models.DateField()
    at = models.TimeField()
    #: Имя IANA пояса, в котором заданы `on` и `at`. Пусто — пояс неизвестен.
