...
from study.models import Bond, Invite, Lesson, Material, Message, Visit
...
#
# Проверяем не то, что путь отвечает, а то, что он отвечает ЧУЖОМУ: id ученика
# преподаватель видит в журнале, id преподавателя ученик видит в кабинете —
# значит подставить чужой id в запрос может кто угодно.
раздел("Разговор заводится связью, а не первым словом:")
r = У.get("/api/study/talks")
разговоры = r.json()["разговоры"]
да("у преподавателя разговор с учеником есть до первого слова",
   [р["кто"] for р in разговоры] == [ученик.id], разговоры)
да("и он пустой", разговоры[0]["последнее"] is None, разговоры[0])
да("имя собеседника названо", разговоры[0]["имя"] == "Аня", разговоры[0])
r = Ч.get("/api/study/talks")
да("у преподавателя без учеников разговоров нет", r.json()["разговоры"] == [], r.json())

раздел("Слова доходят:")
r = У.post(f"/api/study/talks/{ученик.id}",
           json.dumps({"текст": "Аня, повторите §4 к вторнику."}),
           content_type="application/json")
да("преподаватель пишет ученику — 201", r.status_code == 201, r.status_code)
да("своё письмо помечено «моё»", r.json()["письмо"]["мой"] is True, r.json())

r = С.get(f"/api/study/talks/{учитель.id}")
да("ученик письмо видит", len(r.json()["письма"]) == 1, r.json())
да("и оно для него НЕ своё", r.json()["письма"][0]["мой"] is False, r.json()["письма"][0])
да("текст доехал целиком",
   r.json()["письма"][0]["текст"] == "Аня, повторите §4 к вторнику.", r.json()["письма"][0])

r = С.get("/api/study/talks")
да("у ученика одно непрочитанное", r.json()["разговоры"][0]["непрочитано"] == 1, r.json())
r = У.get("/api/study/talks")
да("у писавшего непрочитанного нет", r.json()["разговоры"][0]["непрочитано"] == 0, r.json())
да("но последнее слово в списке видно",
   r.json()["разговоры"][0]["последнее"]["текст"].startswith("Аня, повторите"), r.json())

r = С.get(f"/api/study/talks/{учитель.id}")
r2 = С.get("/api/study/talks")
да("одно чтение ленты непрочитанное НЕ гасит (GET ничего не меняет)",
   r2.json()["разговоры"][0]["непрочитано"] == 1, r2.json())
r = С.post(f"/api/study/talks/{учитель.id}/read")
да("ученик отмечает прочитанным", r.json().get("отмечено") == 1, r.json())
r = С.get("/api/study/talks")
да("непрочитанное обнулилось", r.json()["разговоры"][0]["непрочитано"] == 0, r.json())

r = С.post(f"/api/study/talks/{учитель.id}",
           json.dumps({"текст": "Хорошо, повторю."}), content_type="application/json")
да("ученик отвечает учителю — 201", r.status_code == 201, r.status_code)
r = У.get(f"/api/study/talks/{ученик.id}")
да("оба письма в одной ленте и по порядку",
   [м["текст"] for м in r.json()["письма"]]
   == ["Аня, повторите §4 к вторнику.", "Хорошо, повторю."], r.json())

раздел("Отказы называют причину:")
r = У.post(f"/api/study/talks/{ученик.id}", json.dumps({"текст": "   "}),
           content_type="application/json")
да("пустое сообщение — отказ словами", r.status_code == 400 and "error" in r.json(), r.json())
r = У.post(f"/api/study/talks/{ученик.id}", json.dumps({"текст": "я" * 2001}),
           content_type="application/json")
да("слишком длинное — отказ словами", r.status_code == 400 and "error" in r.json(), r.status_code)
r = У.get(f"/api/study/talks/{ученик.id}")
да("отказанное в ленту не легло", len(r.json()["письма"]) == 2, len(r.json()["письма"]))

раздел("Чужое не отдаём и в него не пишем:")
r = С2.get(f"/api/study/talks/{учитель.id}")
да("посторонний получает «нет такого», а не «нельзя»", r.status_code == 404, r.status_code)
r = С2.post(f"/api/study/talks/{учитель.id}", json.dumps({"текст": "привет"}),
            content_type="application/json")
да("и написать не может — тоже 404", r.status_code == 404, r.status_code)
r = гость.get(f"/api/study/talks/{учитель.id}")
да("гость без учётной записи — 401", r.status_code == 401, r.status_code)
r = гость.get("/api/study/talks")
да("и списка разговоров ему не видно — 401", r.status_code == 401, r.status_code)
r = У.delete(f"/api/study/talks/{ученик.id}")
да("чужой метод — 405 со словами", r.status_code == 405 and "error" in r.json(), r.status_code)

раздел("Зов на занятие — реплика в том же разговоре:")
r = У.post(f"/api/study/talks/{ученик.id}", json.dumps({"урок": урок["id"]}),
           content_type="application/json")
да("преподаватель зовёт на своё занятие — 201", r.status_code == 201, r.status_code)
зов = r.json()["письмо"]
да("вид реплики — зов", зов["вид"] == "invite", зов)
да("в зове есть название занятия", зов["урок"]["название"] == урок["название"], зов)
да("и код комнаты, чтобы войти", зов["урок"]["код"] == код, зов)
r = С.get(f"/api/study/talks/{учитель.id}")
да("зов пришёл в ту же ленту, а не в отдельную",
   r.json()["письма"][-1]["вид"] == "invite", r.json()["письма"][-1])

r = С.post(f"/api/study/talks/{учитель.id}", json.dumps({"урок": урок["id"]}),
           content_type="application/json")
да("ученик звать на занятие не может — 403", r.status_code == 403, r.status_code)
r = Ч.post("/api/study/lessons", json.dumps(
    {"название": "Чужое занятие", "дата": "2026-09-04", "время": "10:00"}),
    content_type="application/json")
чужойУрок = r.json()["урок"]["id"]
r = У.post(f"/api/study/talks/{ученик.id}", json.dumps({"урок": чужойУрок}),
           content_type="application/json")
да("на ЧУЖОЕ занятие не позвать — «нет такого»", r.status_code == 404, r.status_code)
...
# 🔴 А вот сказанное НЕ уходит: занятие сняли, но реплика «зову вас на…»
# остаётся в разговоре и честно показывает, что занятия больше нет. Стирать
# чужие слова задним числом хуже, чем показать, что они устарели (SET_NULL).
зовы = Message.objects.filter(kind=Message.INVITE)
да("зов на снятое занятие из разговора НЕ исчез", зовы.count() == 1, зовы.count())
да("и честно говорит, что занятия больше нет", зовы.first().lesson_id is None, зовы.first().lesson_id)
