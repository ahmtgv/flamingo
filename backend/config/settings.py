...
#
# ⚠️ ЭТО НЕ ОКОНЧАТЕЛЬНОЕ РЕШЕНИЕ, а наименее неверное умолчание. Правильный ответ — пояс
# у человека: СНГ тянется на одиннадцать поясов, и ученик во Владивостоке живёт на семь
# часов впереди Москвы. Поля часового пояса у пользователя нет — вопрос владельцу задан.
# Хранение не меняется: `USE_TZ = True`, в базе по-прежнему UTC; меняется только то, где
# проходит граница суток.
