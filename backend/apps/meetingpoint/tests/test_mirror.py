...
def test_the_mirror_carries_exactly_the_owners_composition():
    ...
    assert {k.value for k in MirrorKind} == {
        ...
        # 🔴 Добавлено решением владельца 17.08 (§27.2): «мои материалы» — личная папка
        # ученика. Белый список §2.2 расширен ЕГО решением, а не трактовкой. Граница при
        # этом не двинулась: сюда попадает только то, что ученик сохранил СЕБЕ САМ, и
        # только ссылкой с заметкой — копии содержимого здесь нет, лицензия остаётся у
        # источника (RND_02 §1).
        "saved",  # §27.2 — что ученик сохранил себе из урока и из хаба источников
    }
