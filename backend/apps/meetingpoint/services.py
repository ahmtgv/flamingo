...
#: 🔴 «ВЕЧНЫЙ УРОК» (находка ревьюера Р-1, 18.08). Занятие «Большой взрыв» началось в 17:16,
#: преподаватель ушёл около 17:30 — в 20:40 продукт всё ещё считал его идущим. Стартовая
#: звала ученика «Войти в урок» в брошенную комнату, саммари не собиралось, зеркало не
#: наполнялось, дневник и посещаемость ждали завершения, которое не наступит.
#:
#: Причина: автозакрытие пропускало занятие, у которого НЕТ связанной машины
#: (`last_seen is None`) — то есть каждое, что ведут из браузера. Оговорка была разумной
#: («не гасить урок, который идёт по серверному пути»), но у неё не было второго конца:
#: занятие без машины не закрывалось НИКОГДА.
#:
#: Здесь этот второй конец. Запас щедрый: преподаватель имеет право задержаться, и обрывать
#: живой урок на пятнадцатой минуте переработки нельзя. Но и четыре часа «идёт» — не запас,
#: а поломка. Забыть «Завершить» — обычное дело, это будет случаться каждую неделю.
LESSON_OVERRUN_LIMIT = dt.timedelta(hours=2)
...
def close_abandoned_sessions(*, now: dt.datetime | None = None) -> list:
    ...
    **Ничего не выдумываем.** `end_at` — последний известный признак жизни машины, а не «сейчас»:
    ...
    for session in live:
        ...
        scheduled_end_no_host = session.start_at + dt.timedelta(minutes=_planned_minutes(session))
        if last_seen is None:
            # У преподавателя нет связанной машины — занятие ведут из браузера (или по
            # серверному контуру, PROMPT_14 §6). Пульса машины здесь не будет никогда, значит
            # судить можно только по расписанию.
            #
            # 🔴 РАНЬШЕ ЗДЕСЬ СТОЯЛ `continue` БЕЗ ВТОРОГО КОНЦА, и такое занятие не
            # закрывалось никогда (находка ревьюера Р-1: «идёт» четвёртый час).
            if moment - scheduled_end_no_host < LESSON_OVERRUN_LIMIT:
                continue
            session.status = SessionStatus.ENDED.value
            # Пишем плановый конец, а не «сейчас»: сколько занятие шло на самом деле, мы не
            # знаем — ровно та же честность, что и в ветке с машиной.
            session.end_at = max(scheduled_end_no_host, session.start_at)
