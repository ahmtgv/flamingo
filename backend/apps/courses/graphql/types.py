...
@strawberry.type
class CourseInviteView:
    """
    Приглашение в курс — код, ссылка и срок.

    ⚠️ Мутации «войти» здесь нет: она отдельная (`redeemCourseInvite`), потому что зовущий и
    входящий — разные люди с разными правами. Смешивать их в одном типе значит однажды отдать
    ученику право раздавать коды.
    """
    ...
    #: 🔴 ССЫЛКИ ЗДЕСЬ НЕТ НАМЕРЕННО. Чтобы вернуть её целиком, серверу нужен публичный
    #: адрес продукта, а зашивать домен в код нельзя — за этим следит караул
    #: `nothing_is_baked_in`, и он меня на этом уже ловил. Клиент знает свой origin и
    #: собирает ссылку сам: `${location.origin}/join/${code}`.
