...
def refresh(refresh_token: str) -> tuple[User, dict[str, str]]:
    ...
    # 🔴 МЕТКА МАШИНЫ ПЕРЕЖИВАЕТ ОБНОВЛЕНИЕ (найдено аудитом 17.08).
    #
    # Здесь стояло `issue_tokens(user)` без `device_id`, и `did` терялся при ПЕРВОМ ЖЕ
    # обновлении. Приложение обновляет сессию на каждом старте, значит уже со второго запуска
    # сессия переставала быть машинной — и `authenticate_request` больше не мог её отклонить.
    #
    # Чем это было: «Отозвать» на листе D8 гасила ключ машины и ОСТАВЛЯЛА живую
    # преподавательскую сессию на украденном ноутбуке на все четырнадцать дней. Кнопка
    # читалась как сделанная и не была — ровно то, что §Б0-септ обещал не допустить.
    #
    # Хуже: сам отзыв вызывал отказ → фронт шёл обновлять сессию → получал пару без `did`,
    # то есть отзыв сам себе и готовил побег.
    return user, issue_tokens(user, device_id=payload.get("did"))
