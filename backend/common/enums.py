...
class EnrollmentStatus(Enum):
    """Жизненный цикл зачисления — и ОТДЕЛЬНО от него `AccessStatus` (оплата, §54.1).

    🔴 Ожиданий два, и они названы по-разному нарочно. Человек, который ждёт согласия
    родителя, и человек, который ждёт ответа преподавателя, делают РАЗНОЕ: первый не может
    ничего, второму нужно только подождать. Один общий `PENDING` заставил бы экран сказать
    обоим «ждите» — то есть не сказать ничего.
    """

    ACTIVE = "active"
    COMPLETED = "completed"
    #: Заявка из каталога: преподаватель ещё не принял (§54.1).
    PENDING_TEACHER = "pending_teacher"
    #: Ученику меньше 16 (§51): согласие даёт законный представитель, и до него доступа нет.
