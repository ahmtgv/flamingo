...
#: 🔴 МЕЛКУЮ КАРТИНКУ НЕ БЕРЁМ ВОВСЕ. Половина сайтов объявляет витриной свой
#: логотип: `pg-logo-144x144.png` у Гутенберга, `osm_logo_256` у карт. Растянуть
#: логотип 144 px на плитку 640 — получить мыло, которое хуже нарисованной
#: обложки. Порог — ровно наш размер: если картинку пришлось бы УВЕЛИЧИВАТЬ,
#: она не витрина. Отказ здесь не беда: у источника просто остаётся рисунок.
МИН_Ш, МИН_В = ШИРИНА, ВЫСОТА

#: 🔴 ПРАВИЛА ПРО КВАДРАТ ЗДЕСЬ БОЛЬШЕ НЕТ, И ЭТО ИСПРАВЛЕНИЕ, А НЕ УПРОЩЕНИЕ.
#: Оно было: «отношение сторон ниже 1,2 — логотип». На боевом обходе 12.09 оно
#: первым делом выбросило снимок «Хаббла» 1120×1120 — настоящую астрономическую
#: фотографию: NASA отдаёт витрину квадратной. Форма логотип не выдаёт, размер
#: выдаёт: все настоящие логотипы поймались порогом выше (144, 256, 380), а
#: большая квадратная картинка — почти всегда фотография, и середина её,
#: обрезанная до 16:9, остаётся видом.
...
def привести(сырое: bytes) -> tuple[bytes, int, int]:
    ...
    try:
        ...
        # Сюда же приходит svg: Pillow его не открывает вовсе, а сайты
        # (Всемирный банк, например) объявляют витриной именно svg-логотип.
        raise НеВышло(f"картинка не открылась ({type(e).__name__})") from None

    ш, в = им.size
    if ш < МИН_Ш or в < МИН_В:
        raise НеВышло(f"мелкая: {ш}×{в} при нужных {МИН_Ш}×{МИН_В} — похоже на логотип")
