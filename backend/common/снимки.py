...
#: 🔴 КВАДРАТ — ТОЖЕ ПОЧТИ ВСЕГДА ЛОГОТИП. Вид мира бывает широким; печать,
#: значок и аватар — квадратные. Обрезать квадрат до 16:9 значит показать
#: поясок логотипа. 1,2 — граница, ниже которой картинка перестаёт быть видом:
#: 1200×630 (обычный og:image) даёт 1,9, наш размер — 1,78, квадрат — 1,0.
МИН_ОТНОШЕНИЕ = 1.2
...
def привести(сырое: bytes) -> tuple[bytes, int, int]:
    ...
    if ш / max(в, 1) < МИН_ОТНОШЕНИЕ:
        raise НеВышло(f"почти квадрат: {ш}×{в} — похоже на логотип, а не на вид")
