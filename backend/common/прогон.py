...
views.СПИСОК.write_text(
    json.dumps([{"id": "проба", "home": "http://x", "name": "Проба", "снимок": True},
                {"id": "реклама", "home": "http://x", "name": "Реклама", "снимок": False}],
               ensure_ascii=False),
    encoding="utf-8")
...
# ── 9. отказ человека действует и на отдаче ──────────────────────────────────
# 🔴 ФАЙЛ МОГ ОСТАТЬСЯ С ПРОШЛЫХ ОБХОДОВ. Если отказ действует только в обходе,
# вчерашняя рекламная карточка так и будет висеть на витрине — отказ на словах.
(снимки.куда(ПАПКА) / "реклама.jpg").write_bytes(было)
снимки.записать_опись(ПАПКА, {"проба": {"взято": сейчас, "байт": len(было)},
                              "реклама": {"взято": сейчас, "байт": len(было)}})
о = c.get("/api/hub/preview/реклама")
проверить("отказанный снимок не отдаётся, даже если файл лежит",
          о.status_code == 404, str(о.status_code))
о = c.get("/api/hub/previews")
есть2 = json.loads(о.content).get("есть", [])
проверить("и в списке его нет", есть2 == ["проба"], str(есть2))
