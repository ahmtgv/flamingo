...
## 0 · Подключиться и обновить код

```
cd /opt/flamingo && git pull
```

**Должно появиться:** список изменённых файлов и строка вроде `Fast-forward`. Если написано
`Already up to date` — значит код уже свежий, это нормально.
