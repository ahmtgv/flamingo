...
⏳ **ЧТО ОСТАЛОСЬ ИЗ БЛОКА 3 (не начато):**
- **R4.1, движок тестов**: `Exercise` / `ExerciseSet(mode=LIVE|HOMEWORK|PRACTICE)` / `Attempt(context)` /
  `SkillMastery` по `RND_01_SPEC_ENGLISH.md` §3 и §7.4. Спека уже спроектировала модель — **не
  проектировать заново**: новую сущность `Grade` не заводить, домашка пишет агрегат в `Submission`,
  живой урок по умолчанию в журнал не идёт, практика — никогда. Живая картина по группе — агрегат
  `{studentId, state, isCorrect, elapsedMs}`, не «кто тупит».
