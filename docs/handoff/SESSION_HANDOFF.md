...
- **Institutions step (c) — admin FE (IN PROGRESS).** Blocker resolved: `me.adminProfile.institution` now gives the admin their institution (`628e786`, resolver from the active admin membership). Remaining: `features/admin` (institution settings + branding, members, groups) mirroring courses/schedule patterns; `ru/institutions.json`; wire the admin cabinet (nav items exist but unlinked, like homework's were). FE-only.
...
- 🟡 **IN PROGRESS — Admin / institutions.** Owner decisions recorded ([`INSTITUTIONS_PLAN.md`](INSTITUTIONS_PLAN.md): Option A group model; reviews→engagement; no `StudentProfile.institution`; back-office onboarding; branding stored-unused). **Backend DONE** — step (a) core `17dcfcd`, step (b) cross-app FKs + access `5f12481`/`ed580af` (see §3).
   1. **Step (c) — admin FE — IN PROGRESS.** Blocker cleared (`628e786`: `me.adminProfile.institution`). Building `features/admin` (institution settings + branding, members, groups) + `ru/institutions.json` + admin-cabinet wiring. FE-only.
