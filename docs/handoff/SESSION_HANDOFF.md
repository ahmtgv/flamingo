...
All green: **backend 44 pytest on Postgres + ruff + black clean; frontend `npm run build` + `npm run lint` + 15 vitest** (run from `backend/` and `frontend/`).
...
1. **Finish the course constructor — CURRENT NEXT TASK** (plan-first; small, backend-ready). Reorder + edit-course UI in `frontend/src/features/courses/ui/CourseDetailScreen.tsx`. The ops/hooks already exist (`courses.graphql`: `UpdateCourse`, `ReorderSections`, `ReorderLessons` → `useUpdateCourseMutation`/`useReorderSectionsMutation`/`useReorderLessonsMutation`); **only the UI is missing**. Add per-section/-lesson move up/down controls (compute reordered id arrays) and an edit-course form. Backend supports all of it (`backend/apps/courses/services.py`). See §4.
