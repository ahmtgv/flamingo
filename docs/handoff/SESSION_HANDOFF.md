...
All green: **backend 52 pytest on Postgres + ruff + black clean; frontend `npm run build` + `npm run lint` + 22 vitest** (run from `backend/` and `frontend/`).
...
- **Institutions step (b) — cross-app FKs (NEXT, owner-gated).** Add nullable `group_id` to `Course`/`LessonSession`/`Homework` + `Course.institution`; expose the SDL cross-app fields; route group→course access **inside `can_access_course`**. This migration **alters existing tables** → must be shown to the owner and approved before applying. Then step (c) = admin FE.
...
- 🟡 **IN PROGRESS — Admin / institutions.** Owner decisions recorded ([`INSTITUTIONS_PLAN.md`](INSTITUTIONS_PLAN.md): Option A group model; reviews→engagement; no `StudentProfile.institution`; back-office onboarding; branding stored-unused). **Step (a) backend core DONE** (`17dcfcd`, see §3).
   1. **Step (b) — cross-app FKs (CURRENT NEXT; owner must approve the migration).** Nullable `group_id` on `Course`/`LessonSession`/`Homework` + `Course.institution`; expose the SDL cross-app fields; group→course access **inside `can_access_course`**. The migration **alters existing tables** — show it and wait for approval before applying.
   2. **Step (c) — admin FE.** `features/admin` (or `institutions`): institution settings + branding, members, groups; `ru/institutions.json`; wire the admin cabinet.
