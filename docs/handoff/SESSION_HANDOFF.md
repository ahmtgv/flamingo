...
1. **Payments/billing seams — NOT IMPLEMENTED (no `billing` app exists).** Payment-readiness rules are now in `CLAUDE.md` → "Future: Payments / Billing" (committed `eb5bcb4`): all access decisions go through a centralized **`courses/access.py: can_access_course(user, course)`**; `Enrollment.access_status` (active / pending_payment, default `active`) and nullable `Course.price`/`Course.currency` (null = free) are the integration points; all payment code lives in a dedicated `billing` app; RU/CIS (future YooKassa, money as integer kopecks, 54-FZ receipts). **Do NOT build payment/pricing/subscription logic yet** — still open (see §8): the monetization model (simple vs marketplace) and per-course vs subscription gating. The allowed first step now is the **seams only**: add `can_access_course` (default-open) + the nullable `access_status`/`price`/`currency` fields, so later gating drops into one place.
