...
All green: **backend 31 pytest on Postgres + ruff + black clean; frontend `npm run build` + `npm run lint` + 12 vitest** (run from `backend/` and `frontend/`).
...
1. **Homework/grades — CURRENT NEXT TASK.** `apps/homework` (`Homework`, `Submission`, `SubmissionFile`, grading) per ERD §3.4 + SDL; FE homework feature. **Must route every "can this student view/submit/be graded on this homework" decision through `courses/access.py: can_access_course`** (first new module since the chokepoint exists — use it, do NOT add ad-hoc access checks). Needs the files module for submission uploads (or stub `fileKeys` for MVP).
