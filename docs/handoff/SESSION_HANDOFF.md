...
All green: **backend 44 pytest on Postgres + ruff + black clean; frontend `npm run build` + `npm run lint` + 12 vitest** (run from `backend/` and `frontend/`).
...
- ✅ **DONE — Homework/grades backend** (`ec3d638`, see §3). `apps/homework` models/services/GraphQL/migration/tests; student access routed through `can_access_course`. **FE not built.**
1. **Homework FE feature — CURRENT NEXT TASK** (plan-first). `frontend/src/features/homework/` mirroring `features/courses`/`schedule`: `graphql/homework.graphql` ops (fragment-free, live fields only — incl. the new `lessonHomework`), regenerated hooks (`npm run codegen`), and screens for **assign** (teacher: create/publish on a lesson, `submissionStats`, grading list via `homeworkSubmissions` → `gradeSubmission`), **submit** (student: `lessonHomework` → `submitHomework` with `contentText`; `fileKeys` stubbed until the files module), and **grades** (student: `viewerSubmission`/`mySubmissions`). New `ru` i18n namespace `homework.json`. Wire into the course/lesson and cabinet UIs.
2. **Finish the constructor** (reorder + edit-course UI) — see §4; small, backend-ready.
3. **Admin / institutions** — `apps/institutions` (`Institution`, memberships, `Group`/`GroupMembership`/`GroupTeacher`, branding, review moderation); resolves the ERD §7 open question (group_id vs CourseGroup M2M) first.
4. **SEduM Lite** — `apps/seedum` + `frontend/src/seedum/` (MediaPipe worker, attention pipeline → aggregates only, UBP in IndexedDB, `reportAttention`, `attentionUpdates` **subscription**). Requires standing up **Django Channels** (`config/asgi.py` is HTTP-only today) and the live CMF room consuming the scheduling LiveKit tokens.
