...
📦 **2026-08-14 — Р5.2: СПАЙК ДЕСКТОПА — 4 коммита (`99f34b9` · `bb98c40` · `a508142` · `8e45360`).**
Гейты: **BE 468→482 pytest** (+14) · **и те же 482 на SQLite** / **FE 370 без изменений** /
ruff · black · migrations · eslint · **build** — чисто.

- **`ADR-001`** — гипотеза промпта выдержала. Три вопроса отвечены прогонами:
  **миграции** — весь домен работает на SQLite, 482 из 482. Проверка была почти бесплатной и никто её
  не делал: `settings_test.py` уже описывал SQLite, но pytest шёл по Postgres. Нативных enum никогда
  не было (`CharField(choices)` с первого дня), JSONB переносится (в JSON никто не лезет лукапом),
  UUID PK переносится. **Одна настоящая поломка, и тихая:** `icontains` на SQLite складывает регистр
  только для ASCII — «биолог» не находил «Биология», пустой список без объяснений.
  **вес и старт** — 0,88 с от запуска до первого ответа GraphQL, 1,1 мс тёплый, кабинет 1,2 МБ,
  зависимости sidecar 118 МБ (из них ~30 МБ — boto3 · redis · celery — десктопу не нужны).
  Размер установщика **не измерял и не выдумывал**: нужна настоящая сборка Tauri.
  **обновление** — одна сборка, одна версия; старая сборка отказывается открывать кабинет, который
  мигрировала новая (откат — опасное направление: ничего не падает, колонки просто значат другое).
- **`bb98c40`** — `settings_desktop.py` (меняет **где**, не **что**; тест грепает, что профиль не
  трогает middleware и авторизацию), Unicode-aware `LIKE` для SQLite, второй бэкенд хранилища
  через **конфигурацию**, `common/desktop.py`.
- **`a508142` — 🔴 красный долг закрыт**: зеркало **физически копирует** файлы работ в
  `mirror/<ученик>/<сдача>/`, а не ссылается на ключ преподавателя. Тест на локальном бэкенде с
  настоящими байтами: удалили учётку преподавателя И убрали исходный файл — сочинение читается.
- **`8e45360` — долг Р5.1 закрыт**: машина аутентифицируется `Authorization: Device <key>`.
  Ключ аргументом клал живой кредентив в каждый лог запросов, где пишутся переменные.

🪞 **2026-08-14 — правка зеркала под §20.4 — коммит `d0a2536`.** Владелец поправил моё правило,
и поправка верна: «зеркало несёт только текст» писалось против видео и аудио ЗАНЯТИЯ, а запретило
заодно и работу ребёнка. Теперь два правила: медиа занятия по-прежнему не проходит
(`recording` · `transcript` · `lesson_video` · `lesson_audio`), но `audio`/`video` сняты с запрета —
они блокировали собственную запись ребёнка, читающего вслух. Файлы работ едут **содержимым**
(имя · ключ объекта · размер) и открываются через `mirroredFileUrl`, авторизованный **по зеркалу**,
а не по сдаче: сдачи уже нет вместе с учёткой преподавателя, в этом и смысл. Безопасность даёт
происхождение, а не тип файла: в зеркало попадают только `SubmissionFile`, привязанные на сдаче
через `assert_caller_key`. §20.4.2 — одна попытка = один документ со всеми ответами внутри.

📡 **2026-08-14 — Р5.1: СИГНАЛИНГ, TURN, ЗАМЕР КАНАЛА — 1 коммит `8180fc1`.**
Гейты: **BE 430→468 pytest** (+38, включая правку зеркала) / **FE 370 без изменений** (фаза серверная + infra) / ruff · black ·
...
- Живая приёмка Р5.1 (два браузера · TURN · битрейт при восьми) — перенесена в Р5.3/Р5.4, на машинах
  двух реальных преподавателей.
- **Из ADR-001 §6:** настоящая сборка Tauri (размер установщика, первый запуск) ·
  `requirements-desktop.txt` минус boto3/redis/celery · PyInstaller против venv · второй писатель
  в SQLite, если Р5.7 приведёт планшет к той же базе.
