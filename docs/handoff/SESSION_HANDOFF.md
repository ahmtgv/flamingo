...
All green: **backend 53 pytest on Postgres + ruff + black clean; frontend `npm run build` + `npm run lint` + 22 vitest** (run from `backend/` and `frontend/`).
...
- **Institutions step (c) — admin FE — ⛔ BLOCKED, needs owner (see §8).** Every institutions query needs an `institutionId`, but **an admin has no way to discover their own institution** in-app: `adminDashboard` is SDL-only/unimplemented, and the live `AdminProfileType` exposes only `user`. Building a usable admin FE first needs a tiny backend enabler — **recommended:** expose `AdminProfileType.institution` by deriving it from the caller's active admin `InstitutionMembership` (a resolver in `accounts/graphql/types.py` — **no migration, no SDL edit**, since the SDL already declares `AdminProfile.institution`). Once approved, step (c) is FE-only: `features/admin` (institution settings + branding, members, groups) mirroring courses/schedule patterns; `ru/institutions.json`; wire the admin cabinet (nav items exist but unlinked, like homework's were).
...
- 🟡 **IN PROGRESS — Admin / institutions.** Owner decisions recorded ([`INSTITUTIONS_PLAN.md`](INSTITUTIONS_PLAN.md): Option A group model; reviews→engagement; no `StudentProfile.institution`; back-office onboarding; branding stored-unused). **Backend DONE** — step (a) core `17dcfcd`, step (b) cross-app FKs + access `5f12481`/`ed580af` (see §3).
   1. **Step (c) — admin FE — ⛔ BLOCKED on a 1-line backend decision (§4/§8).** Admin can't discover their institution id; recommended enabler = expose `AdminProfileType.institution` (derive from active admin membership; no migration/SDL change). Approve, then step (c) is FE-only.
2. **SEduM Lite** — `apps/seedum` + `frontend/src/seedum/` (MediaPipe worker, attention pipeline → aggregates only, UBP in IndexedDB, `reportAttention`, `attentionUpdates` **subscription**). Requires standing up **Django Channels** (`config/asgi.py` is HTTP-only today) and the live CMF room consuming the scheduling LiveKit tokens.
...
- ⛔ **Admin institution discovery (blocks institutions step (c) — admin FE):** an admin has no way to find their own `institutionId` (all institutions queries are id-scoped; `adminDashboard` unimplemented; `AdminProfileType` exposes only `user`). **Recommended:** resolve `AdminProfileType.institution` from the caller's active admin `InstitutionMembership` — no migration, no SDL edit (the SDL already declares `AdminProfile.institution`). One-word approval unblocks the admin FE.
