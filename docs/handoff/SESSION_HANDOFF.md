...
All green: **backend 44 pytest on Postgres + ruff + black clean; frontend `npm run build` + `npm run lint` + 21 vitest** (run from `backend/` and `frontend/`).
...
1. **Admin / institutions — CURRENT NEXT TASK (plan written, awaiting owner review).** `apps/institutions` (`Institution`, memberships, `Group`/`GroupMembership`/`GroupTeacher`, branding, review moderation). **Blocked on a product decision** — the ERD §7 open question (group_id on COURSE/LESSON_SESSION vs a `CourseGroup` M2M) must be decided first (§8). A plan-only investigation is in [`docs/handoff/INSTITUTIONS_PLAN.md`](INSTITUTIONS_PLAN.md) — read it before implementing.
