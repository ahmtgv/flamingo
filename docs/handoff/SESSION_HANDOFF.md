...
- **Institutions step (c) — admin FE (NEXT).** No backend blocker: the institutions GraphQL (queries/mutations) is live. Build `features/admin` (institution settings + branding, members, groups) mirroring the courses/schedule feature patterns; `ru/institutions.json`; wire the admin cabinet (the admin nav items exist but are unlinked, like the homework ones were). FE-only.
...
- 🟡 **IN PROGRESS — Admin / institutions.** Owner decisions recorded ([`INSTITUTIONS_PLAN.md`](INSTITUTIONS_PLAN.md): Option A group model; reviews→engagement; no `StudentProfile.institution`; back-office onboarding; branding stored-unused). **Backend DONE** — step (a) core `17dcfcd`, step (b) cross-app FKs + access `5f12481`/`ed580af` (see §3).
   1. **Step (c) — admin FE (CURRENT NEXT).** `features/admin` (or `institutions`): institution settings + branding, members, groups; `ru/institutions.json`; wire the admin cabinet. FE-only; backend GraphQL is live.
